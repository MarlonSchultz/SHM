...
export function useAddProject(draft: DraftProject): [() => Promise<boolean>, ApiResourceState<unknown>] {
    const [startRequest, apiResourceState] = useApiResourcePost<unknown, DraftProject>('/projects');

    return [
        () =>
            startRequest(draft)
                .then(([, response]: [unknown, Response]) => response.status === 201)
                .catch(() => false),
        apiResourceState,
    ];
}
